...
- 2. [EC2 <--> S3（不使用 S3 终端节点）](#2-ec2----s3不使用-s3-终端节点)
  ...
  - 2.3 [EC2 通过 NAT-GW 访问 S3](#23-ec2-通过-NAT-GW-访问-s3)
...
### 2.3 EC2 通过 NAT-GW 访问 S3
当 EC2 实例部署在私有子网内、且没有部署 S3 endpoint 时，EC2 实例必须通过 NAT Gateway 访问 S3 中资源。除了 2.1、2.2 中介绍的成本外，还会产生 NAT Gateway 的流量处理费。

![Traffic via NAT gateway](png/02.05-EC2-NAT-S3.png)  

关于 NAT Gateway 的流量处理费用计算信息，可参考 [这里](../../Networking/VPC/VPC-CN.md#2-nat-gateway)。
